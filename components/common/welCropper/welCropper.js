...
Component({
    ...
    methods: {
        ...
        showCropper: function (options) {
            ...
            cropperData.ratio = options.ratio
        ...
        loadImage: function (src, width, height, isRotate) {
            ...
            updateData.cropperMovableItems = {
                ...
                bottomleft: {
                    x: 50,
                    y: 50 + (150 / this.data.cropperData.ratio)
                },
                bottomright: {
                    x: 200,
                    y: 50 + (150 / this.data.cropperData.ratio)
        ...
        drawLines: function (cropperMovableItems, imageInfo, rotateDegree, callback) {
            ...
            // }
            // else {
            //     if(rect.h + rect.x > size.width){
            //         rect.h = size.width - rect.x
            //         rect.w = rect.h
            //     }else{
            //         rect.w = rect.h
            //     }
        ...
        setupMoveItem: function (key, changedTouches, imageInfo, callback) {
            ...
            if (changedTouches.length == 1) {
                ...
                let distanceX = Math.round(x - cropperMovableItems[key].x)
                let distanceY = Math.round(y - cropperMovableItems[key].y)
                
                if(distanceX !== 0 && distanceY !== 0){
                    // let distance = Math.round(Math.sqrt(distanceX*distanceX + distanceY*distanceY))
                    let distance = Math.round(Math.abs(distanceX * 1.62))
                    if(key == 'bottomright' || key == 'topleft'){
                        cropperMovableItems[key].x = cropperMovableItems[key].x + Math.round(distance / 1.62) * (distanceX / Math.abs(distanceX))
                        cropperMovableItems[key].y = cropperMovableItems[key].y + Math.round(distance / 1.62) * (distanceX / Math.abs(distanceX) / cropperData.ratio)
                ...
                if (mode == 'rectangle') {
                    ...
                    if (key == 'topleft') {
                        cropperMovableItems['bottomleft'].x = cropperMovableItems[key].x
                        cropperMovableItems['topright'].y = cropperMovableItems[key].y
                    } else if (key == 'topright') {
                        cropperMovableItems['bottomright'].x = cropperMovableItems[key].x
                        cropperMovableItems['topleft'].y = cropperMovableItems[key].y
                    } else if (key == 'bottomleft') {
                        cropperMovableItems['topleft'].x = cropperMovableItems[key].x
                        cropperMovableItems['bottomright'].y = cropperMovableItems[key].y
                    } else if (key == 'bottomright') {
                        cropperMovableItems['topright'].x = cropperMovableItems[key].x
                        cropperMovableItems['bottomleft'].y = cropperMovableItems[key].y
                    }
                }
                ...
                // console.log('cropperMovableItems', cropperMovableItems)
        ...
        moveBox(e){
            let offsetX = e.touches[0].clientX - touchStartX
            let offsetY = e.touches[0].clientY - touchStartY
            let cropperMovableItems = this.data.cropperMovableItems
            for (const key in cropperMovableItems) {
                cropperMovableItems[key].x += offsetX
                cropperMovableItems[key].y += offsetY
            }
            ...
            this.drawLines(cropperMovableItems, {
                path: z.data.cropperData.imageInfo.path,
                width: originalSize.width,
                height: originalSize.height
            }, cropperChangableData.rotateDegree, (canCrop) => {
                cropperChangableData.canCrop = canCrop
                z.setData({
                    cropperChangableData: cropperChangableData,
                    cropperMovableItems: cropperMovableItems
