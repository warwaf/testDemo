...
Page({
    ...
    changeScenes: function (e) {
        let scenes = e.target.dataset.scenes;
        if(scenes == this.data.scenes) return false;
        this.setData({
            scenes: scenes,
        })
        this.reflesh();
    ...
    async onLoad(options) {
        options.room_no = options.activityId
        if(options.scene){
        ...
        wx.setNavigationBarTitle({
            title: `${options.scenes ? options.scenes :'会场'}` 
        })
      
        let secnsList = wx.getStorageSync('PATHCONFIG');
        this.setData({
            pathOptions: options,
            secnsList: secnsList,
    ...
    async _initRoomConfig(){
        ...
        if(data && data.watermarkImg){
            app.globalData.waterMark = data.watermarkImg.slice(data.watermarkImg.indexOf('p1.hucai.com'))
        }else{
    ...
    /**
     * 分页加载图片数据 this.data.pathOptions.activityId this.data.scenes
     */
    
    _loadPhotoData() {
        return new Promise((resolve, reject) => {
            homeModel.getRoomPhotos(this.data.scenes, this.data.pathOptions.activityId,this.data.pageNo++, this.data.size * 3).then(res => {
                if (this.data.pageNo == 2) {
                    this.setData({
                        isEmpty: false,
                        imgListLeng: res.result.list.length == 0 ? false : true
                    })
                    //判断相册是否为空
                    console.log(res.result.list.length, 'res.result.list')
                ...
                setTimeout(() => {
                    if(this.data.activityInfo.bannerImg.includes('hcmtq.oss-cn-hangzhou.aliyuncs.com/Banner')){
                        this._changeBanner(res.result.list)
                    }
                }, 2000)
                resolve()
            })
    ...
    showDetail: function(event) {
        ...
        wx.navigateTo({
            url: `/pages/album/detail/detail?index=${currentIndex}&room_no=${app.globalData.roomInfo.room_no}&newActivity=1`
        })
    },
