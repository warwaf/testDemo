...
Page({
  ...
  submit() {
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        if (_res.code == 200) {
          wx.redirectTo({
            url: `/pages/activity/scheduling/schedulingDetail/schedulingDetail?orderNo=${app.globalData.schedulUserInfo.no}`
          })
        } else {
  ...
  toCheck() {
    ...
    wx.navigateTo({
      url: `/pages/activity/scheduling/checkSchedule/checkSchedule?photoDate=${this.data.photoDate}&storeNo=${this.data.storeInfo.storeNo}&storeName=${this.data.storeInfo.storeName}`,
    })
  },
  /**
   * 控制弹窗
