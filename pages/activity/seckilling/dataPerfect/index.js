        ...
        wx.request({
          ...
          success: res => {
              if (res.data.code==200) {
                if (res.data.result) {
                  wx.navigateTo({
                    url: res.data.result.activityMsOrder.appointmentType==0?'/pages/activity/scheduling/scheduling?no='+res.data.result.activityMsOrder.orderNo:'/pages/activity/scheduling/schedulingDetail/schedulingDetail?orderNo='+res.data.result.activityMsOrder.orderNo,
                  })
                } else {
                  wx.showToast({
                    title: '订单正在处理',
